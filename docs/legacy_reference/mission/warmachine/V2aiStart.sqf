...
{
	{
		...
		call
		{
			if(side _x==sideW) exitWith {if((_x distance posBaseW1 > 200)&&(_x distance posBaseW2 > 200)&&(_x distance plHw > 200))then{_t=false;};};
			if(side _x==sideE) exitWith {if((_x distance posBaseE1 > 200)&&(_x distance posBaseE2 > 200)&&(_x distance plHe > 200))then{_t=false;};};
		};
	} forEach allPlayers;
