...
}else
{
	...
	if(progress>1)then
	{
		...
		{
			if(_unit isKindOf "AIR" && _plHWDefined)then{_post=[plHW];}else
			{
			...
			if(count _post==0 && _plHWDefined)then{_post=[plHW];};
		}else //sideE
		{
			if(_unit isKindOf "AIR" && _plHEDefined)then{_post=[plHE];}else
			{
			...
			if(count _post==0 && _plHEDefined)then{_post=[plHE];};
		};
